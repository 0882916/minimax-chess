class GameObject extends HTMLElement {
    ...
    public update(): void {
        ...
        for (let i = 0; i < 2; i++) {
            if (Math.abs(this.targetPos[i] - this.pos[i]) <= this.speed[i]) this.pos[i] = this.targetPos[i]
            else this.moving = true
        }

        // move if not at target pos (in chess play style)
        if (this.pos[0] > this.targetPos[0]) this.pos[0] -= this.speed[0]
        else if (this.pos[0] < this.targetPos[0]) this.pos[0] += this.speed[0]
        else if (this.pos[1] > this.targetPos[1]) this.pos[1] -= this.speed[1]
        else if (this.pos[1] < this.targetPos[1]) this.pos[1] += this.speed[1]
